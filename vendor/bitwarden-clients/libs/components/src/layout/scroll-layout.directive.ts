...
export class ScrollLayoutDirective extends CdkVirtualScrollable implements OnInit {
  ...
  override getElementRef(): ElementRef<HTMLElement> {
    return this.service.scrollableRef() ?? this.elementRef;
  }

  override measureBoundingClientRectWithScrollOffset(
    from: "left" | "top" | "right" | "bottom",
  ): number {
    const scrollableRef = this.service.scrollableRef() ?? this.elementRef;
    return (
      scrollableRef.nativeElement.getBoundingClientRect()[from] -
