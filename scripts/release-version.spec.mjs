...
function fixture(versions) {
  ...
  mkdirSync(join(root, "apps/macos-autofill/Config"), { recursive: true });
  ...
  writeFileSync(
    join(root, "apps/macos-autofill/Config/Native.xcconfig"),
    `MARKETING_VERSION = ${versions.nativeAutoFill}\n`,
  );
  return root;
}
...
test("rejects a native AutoFill extension version that drifts from the app", () => {
  const root = fixture({
    package: "1.2.3",
    tauri: "1.2.3",
    cargo: "1.2.3",
    lock: "1.2.3",
    nativeAutoFill: "1.2.2",
  });
  assert.throws(() => assertReleaseVersionSync(root), /release version mismatch/);
});
