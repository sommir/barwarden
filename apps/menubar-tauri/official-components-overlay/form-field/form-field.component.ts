...
export class BitFormFieldComponent implements AfterViewChecked {
  ...
  ngAfterViewChecked(): void {
    ...
    if (error) {
      this.input().setAriaDescribedBy(error.id);
    } else if (hint) {
      this.input().setAriaDescribedBy(hint.id);
    } else {
      this.input().setAriaDescribedBy(undefined);
    }
  }
}
