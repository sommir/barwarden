...
fn browser_script(bundle_id: &str, family: BrowserFamily) -> String {
    let tab_property = match family {
        BrowserFamily::Safari => "current tab",
        BrowserFamily::Chromium => "active tab",
    };
    format!("tell application id \"{bundle_id}\" to return URL of {tab_property} of front window")
}
...
impl ActiveTabReader for MacActiveTabReader {
    fn read_url(
        ...
        run_osascript(&browser_script(&target.bundle_id, family), SCRIPT_TIMEOUT)
    }
}
...
mod tests {
    ...
    fn isolated_script_runner_returns_bounded_output() {
        assert_eq!(
            run_osascript(
                "return \"https://login.example.com/account\"",
    ...
    fn isolated_script_runner_maps_tcc_denial_without_crashing_the_host() {
        assert_eq!(
            run_osascript("error number -1743", Duration::from_secs(2)),
