...
- Folder add/edit sheets accept the invoking trigger without breaking legacy `openFor(folder?)` calls. The folder-name input remains initial focus; cancel/Escape restores the invoking trigger.
...
- Brief 8-suite command, including recovery guard: **8 files, 94 tests passed**.
