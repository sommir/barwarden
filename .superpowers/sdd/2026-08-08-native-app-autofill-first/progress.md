...
- Revised schema-v2 PASS evidence accepts exactly three positive codes and requires all 18 current-macOS live rows; macOS 13–25 runtime is fixed as `NATIVE_AUTOFILL_LOWER_OS_RUNTIME_UNVERIFIED` and non-blocking, while deployment/API/binary-floor checks remain strict. Sanitized evidence is generated in private staging before the exact five-item candidate set is exposed by one rename; evidence failure leaves no release directory.
...
- No signed/notarized/stapled product exists. All 18 current-machine live gates remain fixed-code blocked, and the prior GUI session remains `loginwindow`-bound. macOS 13–25 runtime is honestly unverified and no longer blocks promotion. Production configuration was not promoted. Task 9 remains unchecked and evidence status is `BLOCKED`.
