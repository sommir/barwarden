...
- PASS evidence accepts exactly four positive codes in runtime and schema. Sanitized evidence is generated in private staging before the exact five-item candidate set is exposed by one rename; evidence failure leaves no release directory. The signing blocker is uniformly `NATIVE_AUTOFILL_SIGNING_IDENTITY_MISSING`.
- External state remains blocked: no login-Keychain signing identity, Provider profile, or notary profile; private-key import and Apple Portal automatic provisioning were rejected before execution as not authorized. No external credential or account state changed.
...
- Final lifecycle fix: normal and repair picker states both expose an awaited `Turn Off AutoFill`; disable persists cleanup-pending and always attempts lock → current-account projection clear → unregister, with startup retry after account restoration. Retry now awaits status/register/probe recovery and remains query-free until setup is ready. Focused App/Setup/Picker verification passes 84 tests; full TypeScript passes 3,529 with 22 skipped; production web build passes with the existing warnings. Rust/native code is unchanged. Production promotion remains NO and external release blockers are unchanged.
