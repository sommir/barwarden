...
- Evidence RED/GREEN now covers the revised 18-row current-macOS live matrix, fixed-code-only content, path/credential exclusion, and refusal to record PASS without both artifact hashes, macOS major 26 evidence, all current-machine live rows, strict verifier success, and production promotion. Schema v2 requires macOS 13–25 runtime to remain explicitly `NATIVE_AUTOFILL_LOWER_OS_RUNTIME_UNVERIFIED`; it is not a promotion blocker, while deployment target 13.0, compile/API availability, and binary minimum-floor verification remain strict gates. BLOCKED evidence may honestly record another host OS, but PASS is accepted only for `26.x`.
...
- PASS-evidence mutation RED/GREEN proves both runtime and JSON Schema reject an otherwise-valid PASS record containing any code outside the exact three-code positive set, claiming a lower-OS runtime result, or carrying macOS `25.x` instead of the required current-machine `26.x` version.
